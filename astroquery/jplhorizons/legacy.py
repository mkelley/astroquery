...
class HorizonsLegacyClass(BaseQuery):
    ...
    def ephemerides_async(self, *, airmass_lessthan=99,
                          ...
                          quantities=legacy_conf.eph_quantities,
        ...
        URL = legacy_conf.horizons_server
    ...
    def _location_to_params(loc_dict):
        ...
        location = {
            ...
            "COORD_TYPE": "GEODETIC",
            "SITE_COORD": "'{}'".format(str(HorizonsLegacyClass._format_site_coords(loc_dict)))
        }
        return location
    ...
    def _format_site_coords(coords):
        ...
        return HorizonsLegacyClass._format_coords(coords)

    @staticmethod
    def _format_id_coords(coords):
        """`id` dictionary to COMMAND parameter's coordinate format."""
        return f"g:{HorizonsLegacyClass._format_coords(coords)}@{coords['body']}"
    ...
    def _parse_result(self, response, verbose=None):
        ...
        if self.query_type == 'ephemerides':
            column_defs = legacy_conf.eph_columns
        elif self.query_type == 'elements':
            column_defs = legacy_conf.elem_columns
        elif self.query_type == 'vectors':
            column_defs = legacy_conf.vec_columns
        else:
...
HorizonsLegacy = HorizonsLegacyClass()
